...
class PartApplicationService(
	...
	@Transactional(readOnly = true)
	fun listAvailability(): List<PartAvailabilityResponse> =
		parts.findAll().map { p ->
			val pendingReserved = reservations.sumPendingReservedQuantity(p.id)
			val available = p.stockQuantity - pendingReserved
			PartAvailabilityResponse(
				...
				pendingReservedQuantity = pendingReserved,
