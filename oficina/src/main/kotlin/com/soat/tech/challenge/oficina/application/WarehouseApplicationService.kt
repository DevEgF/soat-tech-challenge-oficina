...
class WarehouseApplicationService(
	...
	@Transactional(readOnly = true)
	fun listAllPendingReservations(): List<PartReservationResponse> =
		reservations.findAllPending().map { r ->
			val name = parts.findById(r.partId).map { it.name }.orElse("")
			PartReservationResponse(
				id = r.id,
				workOrderId = r.workOrderId,
	...
	fun confirmStockExitForWorkOrder(workOrderId: UUID) {
		val wo = workOrders.findById(workOrderId).orElseThrow { NotFoundException("Work order not found") }
		reservations.confirmPendingForWorkOrder(workOrderId)
		if (wo.status == WorkOrderStatus.AWAITING_PARTS_RELEASE) {
			wo.startExecution(clock.instant())
			workOrders.save(wo)
		}
	}
