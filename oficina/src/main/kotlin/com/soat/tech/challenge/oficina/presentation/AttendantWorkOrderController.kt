...
class AttendantWorkOrderController(
	private val workOrders: WorkOrderApplicationService,
) {

	@PostMapping
	@ResponseStatus(HttpStatus.CREATED)
	fun create(@Valid @RequestBody req: CreateWorkOrderRequest): WorkOrderResponse =
	...
	fun sendQuoteToCustomer(@PathVariable id: UUID): WorkOrderResponse =
		workOrders.sendQuoteToCustomer(id)

	@PostMapping("/{id}/voltar-diagnostico")
	fun returnToDiagnosis(@PathVariable id: UUID): WorkOrderResponse =
		workOrders.returnToDiagnosis(id)

	@PostMapping("/{id}/registrar-entrega")
	fun registerDelivery(@PathVariable id: UUID): WorkOrderResponse =
