...
class WorkOrderTest {
	...
	inner class GivenNewWorkOrder {
		...
		fun happyPath() {
			...
			wo.startExecution(t0.plusSeconds(170))
			assertEquals(WorkOrderStatus.IN_EXECUTION, wo.status)
			assertNotNull(wo.workStartedAt)
		...
		@Test
		@DisplayName("when startExecution before customer approval then throws")
		fun startExecutionWrongStatus() {
			val wo = WorkOrder.create(
				customerId = UUID.randomUUID(),
				vehicleId = UUID.randomUUID(),
