...
class WarehouseApplicationServiceTest {
	...
	private val fixedInstant = Instant.parse("2026-04-27T12:00:00Z")
	private val clock = Clock.fixed(fixedInstant, ZoneOffset.UTC)
	private val service = WarehouseApplicationService(reservations, workOrders, parts, clock)
	...
	inner class Confirm {
		...
		@Test
		@DisplayName("when work order not in AWAITING_PARTS_RELEASE then only confirms reservations")
		fun delegates() {
			val id = UUID.randomUUID()
			val wo = WorkOrder.create(
				id = id,
			...
			verify(exactly = 0) { workOrders.save(any()) }
			assertEquals(WorkOrderStatus.RECEIVED, wo.status)
		}

		@Test
		@DisplayName("when work order in AWAITING_PARTS_RELEASE then transitions to IN_EXECUTION")
		fun policyStartsExecution() {
			val id = UUID.randomUUID()
			val wo = WorkOrder.create(
				id = id,
				customerId = UUID.randomUUID(),
				...
				approvedAt = fixedInstant.minusSeconds(60),
			)
			every { workOrders.findById(id) } returns Optional.of(wo)
			...
			assertEquals(fixedInstant, wo.workStartedAt)
		}
	}
